...
**`moomoo接続確認.bat` をダブルクリック**します。メニューで口座を選ぶと、
...
```
OpenD is up and your SIMULATE account answers through it.
...
| `no REAL account ... (found: SIMULATE)` | 模擬口座しか無い／実口座がその市場で未承認 | 口座の開設状況を moomoo 側で確認する |
