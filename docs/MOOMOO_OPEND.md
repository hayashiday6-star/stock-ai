...
**列どうしの関係（実測。公式には定義がありません）**

`US.AAPL` の日足 22 営業日分で確認したところ、全行で次が成立しました。

```
super + big + mid + sml = in_flow      22/22 行
super + big             = main_in_flow 22/22 行
```

公式は `main_in_flow` を「主力大口純流入額」とするだけで構成を定義していない
ため、**仕様ではなく観測**として扱ってください。読み方としては、4つの帯が
全体（`in_flow`）を構成し、`main_in_flow` はそのうち特大口＋大口の小計です
（5つ目の帯ではありません）。
...
なく、日本株の相場データが API で提供されていないためです。米国株が同じ接続で
問題なく返ることを確認済みなので、口座側の問題ではありません。実際に返るのは
次のメッセージです。
