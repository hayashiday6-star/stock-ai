...
class ExDateCoverage:
    ...
    **列ごとに独立に数える。** 行が読めたことと、`ExDate` が埋まっていることは
    別である（`docs/POSTMORTEMS.md`「0 を『読めた』と読まない」）。
    """
...
class AnnouncedExDates:
    ...
    **件数だけ返すと、次の一手が打てない**（`docs/POSTMORTEMS.md`「『見ること』と
    書いただけで、見る道具を置いていないか」）。
    """
    ...
    def warnings(self) -> list[str]:
        ...
        # 紛れる（`docs/POSTMORTEMS.md`「合計だけ出すと、その中に紛れる」）。
...
def raw_rows(
    ...
    **読み口が捨てている列は、読み口からは見えない**（`docs/POSTMORTEMS.md`）。
...
class RowCensus:
    ...
    **全体も一緒に持つ。** 絞った先の分布だけを見ても何も言えない
    ——`docs/POSTMORTEMS.md`「件数ではなく割合を出す」。**同じ列が全体でも空なら、
    それはその集合の特徴ではない。**
