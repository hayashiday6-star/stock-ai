...
#: `docs/POSTMORTEMS.md`「幅を決め打つときは、その数字がどこから来たかを書く」。
#:
...
def overlap_ceiling(window: int) -> float:
    ...
    **推定できないときに、推定値の代わりに置く。** 当てにならない推定を
    掛けるより、**安全側の値を置いて「これは上限である」と書く**ほうが正直で
    ある（2026-09-21、ユーザーの案）。

    ## なぜ ``√W`` か

    日次リターンが独立なら、``W`` 日の窓を**間隔 s で**入ったときの平均の
    標準誤差の膨張は ``√(W/s)`` になる。**s が小さいほど大きい**ので、
    **毎日入る（s=1）ときの ``√W`` がどの入り方でも超えない値**である。

    ## 測って確かめた

    独立な日次から窓を作り、平均の SD を分割標本で実測した
    （`tests/test_power.py`）。
    ...
    | 10 | 1 | 2.89 | 3.16 | 3.16 | **2.65** |

    **``√(W/s)`` は上限ではない**——W=20・s=5 で実測 2.22 が式の 2.00 を
    超えた。だから**採らない。** ``√W`` は4行とも超えていない。
    ...
    **日次そのものに自己相関があれば、``√W`` を超えうる。** ここが前提に
    しているのは独立な日次だけである——**そう書いておく**（`docs/POSTMORTEMS.md`
    ...
    Returns:
        膨張の上限。``window`` が 1 以下なら 1.0（重ならない）。
    """
    return math.sqrt(window) if window > 1 else 1.0
...
def required_information_ratio(line: float, inflation: float, years: float) -> float:
    ...
    **`線 ÷ √年数` と書きたくなるが、それは緩む向きの取り違えである。**
    ...
    `docs/POSTMORTEMS.md`「同じ式を3つ書けば、1つは間違える」——**短いものほど、
    違いに気付けない。**
