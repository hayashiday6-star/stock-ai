...
#: 取り逃していた。
#:
#: **「これより前は今後も直せない」と書いてあったが、それは誤りだった**
#: （2026-09-12 に訂正）。Premium の20年で、日付ごとの名簿も廃止銘柄の株価も
#: **2006-09 まで遡れる。** 直せない理由は契約であって、データの不在ではな
#: かった。
#:
#: **それでもこの値は動かさない。** #6 は封印済みで、判定は一度きりである。
#: ここを広げれば、それは同じ説の2回目の判定になる。**#6 は効果そのものが
#: 無かった側**（下げた銘柄が反発するどころか最も低かった）なので、期間を
#: 伸ばしても向きは変わらない——`docs/HYPOTHESES.md` の「20年に伸びたとき、
#: どれが戻ってくるか」を見ること。
